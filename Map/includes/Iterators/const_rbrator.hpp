...
class	ft::const_rbrator
{
	...
	public:
		...
		template < class U > const_rbrator(const ft::rbrator<U>& mit)
		{
			p = mit.base();
		};
		~const_rbrator() {};
		...
		bool	operator==(const ft::const_rbrator<T>& rhs) const
		{
			return (p == rhs.base());
		};
		bool	operator!=(const ft::const_rbrator<T>& rhs) const
		...
		difference_type	operator-(const ft::const_rbrator<T>& rhs) const
		{
